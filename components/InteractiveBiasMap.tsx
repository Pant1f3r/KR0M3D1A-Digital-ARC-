...
export const InteractiveBiasMap: React.FC<InteractiveBiasMapProps> = ({ 
    ...
    const connections = useMemo(() => {
        ...
        const points = anomalies.map(a => ({ id: a.id, x: a.x, y: a.y }));
        const lines: { from: Anomaly['id'], to: Anomaly['id'] }[] = [];
        const processed = new Set<string>();

        for (let i = 0; i < points.length; i++) {
            const currentPoint = points[i];
            const neighbors = points
                .filter(p => p.id !== currentPoint.id)
                .map(p => ({
                    ...p,
                    dist: Math.sqrt(Math.pow(p.x - currentPoint.x, 2) + Math.pow(p.y - currentPoint.y, 2))
                }))
                .sort((a, b) => a.dist - b.dist)
                .slice(0, 2); // Connect to 2 nearest neighbors
            
            for (const neighbor of neighbors) {
                const lineId = [currentPoint.id, neighbor.id].sort().join('-');
                if (!processed.has(lineId)) {
                    lines.push({ from: currentPoint.id, to: neighbor.id });
            ...
            <svg viewBox="0 0 1200 600" className="w-full h-full">
                ...
                <g>
                    {connections.map((conn, index) => {
                        const fromPoint = anomalies.find(a => a.id === conn.from);
                        const toPoint = anomalies.find(a => a.id === conn.to);
                        if (!fromPoint || !toPoint) return null;

                        const isHighlighted = selectedAnomaly && (selectedAnomaly.id === conn.from || selectedAnomaly.id === conn.to);
